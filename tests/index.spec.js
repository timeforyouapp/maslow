import { ModuleCreator, createStoreByModules, createActionFn } from '../src/index';
...
describe('index', () => {
  ...
  const checkCallOutsFor = ({
    ...
    setTimeout(() => {
      ...
      if (stateCheck) {
        stateCheck(state[prop]);
      } else {
        expect(state[prop]).toHaveProperty(expectProp, fakeUserReturn);
      }

      if (userEntryCheck) {
        userEntryCheck(fakeUserApi[api]);
      } else {
        expect(fakeUserApi[api]).toHaveBeenCalledWith(fakeUserEntry);
      }
  ...
  it('should exec inner actions', () => {
    ...
    const dispatchAction = createActionFn(fakeStore, {
      [modlName]: {
