...
import { ReadOnlyProp, MethodNotAFunction } from '../../src/error';
...
describe('MaslowSchema', () => {
  ...
  it('should .set, .next, .prev methods work correcty', () => {
    ...
    expect(() => user.set('id', 2)).toThrowError(ReadOnlyProp());
  ...
  it('should .method add a new method', () => {
    ...
    User.addMethod('foo', function test() {
      this.name = value;
  ...
  it('should throw exception to .method adding a not function', () => {
    ['text', 1, 1.1, true, undefined, null, {}, []].forEach((entry) => {
      expect(() => User.addMethod('foo', entry)).toThrowError(MethodNotAFunction());
    });
  });
  ...
  it('should .validate date', () => {
    ...
    return user.validate().then((errors) => {
      expect(errors).not.toHaveProperty('createdAt');
      expect(errors.fooBar[0]).toHaveProperty('type', 'base');
    });
  });

  it('should .validate throw errors', () => {
    const user = User({
      email: 'gui_x.com', name, id, notAllowedProp,
    });
